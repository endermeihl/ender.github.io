...
def auto_login_hi():
    values = {'card_no': '157949161', 'card_pwd': '622543'}
    ...
    request.add_header('Referer', 'http://localhost:4019/Login.aspx')
    ...
    get_url = 'http://www.safetyme.cn//a/exam.shtml?method=index'  # 利用cookie请求访问另一个网址
    ...
    for link in soup.find_all('a'):
        id=link.get('onclick').replace("openPaper('", '').replace("','3')", "")
        print(id)
        ids.append(id)
    for id in ids[1:2]:
        ...
        i=0
        examtk=[]
        for div in zip(soup.find_all("div", "exam-content"),soup.find_all("div","for-wrong")):
            i=i+1
            st=div[0]
            da=div[1]
            sttype=st.p.span.string
            print(sttype.replace("[","").replace("]",""))
            repstring='<p><span style="color: green; font-weight: bold;">'+sttype+"</span>"+str(i)+"、"
            ...
            for li in st.ul:
                ...
                for inpt in zip(soup2.find_all("input"),soup2.find_all("span")):
                    print(inpt[0]['value'])
                    print(inpt[1].string)
            tm={}
            tm["stlx"] = sttype.replace("[", "").replace("]", "")
            tm["stnr"] = str(st.p).replace(repstring,"").replace("(1.0分)</p>","")
